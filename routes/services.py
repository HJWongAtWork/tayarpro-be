...
from models import Service, User, ServiceType
...
@router.get('/service/{service_id}',  summary="List service based on the service_id", tags=["Services"])
async def get_service_by_id(service_id: str, db: db_dependency):
...
async def get_service_by_type(service_type: str, db: db_dependency):
    service = db.query(Service).filter(
        Service.typeid == service_type).all()
    if not service:
        raise HTTPException(status_code=404, detail="Service not found")
    return service
...
async def get_service_by_description(description: str, db: db_dependency):
    services = db.query(Service).filter(
        Service.description == description).all()
    if not services:
